...
contract VaultFactory is IVaultFactory, Ownable {
    ...
    address public dittoBridgeReceiver;

    /// @dev entryPoint creator address
    address public entryPointCreator;

    /// @dev entryPoint address
    ...
    /// @inheritdoc IVaultFactory
    function setEntryPointCreatorAddress(
        address _entryPointCreator
    ) external onlyOwner {
        entryPointCreator = _entryPointCreator;
    }
    ...
    function addStake(uint32 _unstakeDelaySec) external payable {
        IStakeManager(entryPoint).addStake{value: msg.value}(_unstakeDelaySec);
    }
    ...
    function withdrawStake(address payable withdrawAddress) external onlyOwner {
        IStakeManager(entryPoint).withdrawStake(withdrawAddress);
    }
    ...
    function crossChainDeploy(
        ...
        if (msg.sender != dittoBridgeReceiver) {
            if (msg.sender != entryPointCreator) {
