...
import {IAutomate, ModuleData, Module, IOpsProxyFactory, IGelato, IProxyModule} from "@gelato/contracts/integrations/Types.sol";
...
contract EntryPointLogic is IEntryPointLogic, BaseContract, DittoFeeBase {
    ...
    IAutomate private immutable _automate;
    ...
    IOpsProxyFactory private immutable _opsProxyFactory;
    ...
    constructor(
        ...
        if (address(automate) != address(0)) {
            IGelato gelato = IGelato(IAutomate(automate).gelato());
            feeCollector = gelato.feeCollector();

            address proxyModuleAddress = IAutomate(_automate)
                .taskModuleAddresses(Module.PROXY);

            opsProxyFactory = IProxyModule(proxyModuleAddress).opsProxyFactory();
        }

        _feeCollector = feeCollector;
        _opsProxyFactory = IOpsProxyFactory(opsProxyFactory);
    }
    ...
    function runGelato(uint256 workflowKey) external {
        ...
        // send fee to gelato
        TransferHelper.safeTransferNative(_feeCollector, fee);
    ...
    /// @inheritdoc IEntryPointLogic
    function dedicatedMessageSender()
        public
        view
        returns (address dedicatedMsgSender)
    {
        (dedicatedMsgSender, ) = _opsProxyFactory.getProxyOf(address(this));
    }
    ...
    function _createTask(
        ...
        moduleData.args[0] = _resolverModuleArg(
            address(this),
            abi.encodeCall(this.canExecWorkflowCheck, (workflowKey))
        );

        bytes memory execData = abi.encodeCall(this.runGelato, (workflowKey));
